from mplee_interpreter.lexer import *
from mplee_interpreter.parser import *
...
class BuiltInFunction(BaseFunction):
    ...
    def execute_max(self, exec_ctx):
        ...
        max_value = list_.elements[0]
        index = 0
        for x in range(0, len(list_.elements)):
            if max_value.value < list_.elements[x].value:
                max_value = list_.elements[x]
                index = x

        return RTResult().success(List([Number(index), max_value]))
